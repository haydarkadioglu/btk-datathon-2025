...
# Önceden eğitilmiş embedding'ler için sentence-transformers kütüphanesini ekliyoruz.
# Kurulum için: pip install sentence-transformers
...
# %%
# ==============================================================================
# 1.5. PRE-TRAINED EMBEDDING'LER İÇİN VERİ SİMÜLASYONU VE OLUŞTURMA
# ==============================================================================
print("\n--- Adım 1.5: Pre-trained Embedding'ler Oluşturuluyor ---")

# Gerçekte bu veri (ürün açıklamaları) veritabanınızdan gelir.
# Burada konsepti göstermek için örnek bir veri seti oluşturuyoruz.
all_product_ids = df_full['product_id'].unique()
mock_descriptions = {pid: f"ürün {pid} için kaliteli ve kullanışlı bir elektronik eşya" for pid in all_product_ids}
df_products = pd.DataFrame(list(mock_descriptions.items()), columns=['product_id', 'description'])
print(f"Simüle edilmiş {len(df_products)} adet ürün açıklaması oluşturuldu.")

# Hata veren model yerine, kimlik doğrulaması gerektirmeyen popüler bir model kullanıyoruz.
# use_auth_token=False parametresi ile ortamdaki olası geçersiz token'ların kullanılmasını engelliyoruz.
print("   - SentenceTransformer modeli yükleniyor (bu işlem biraz sürebilir)...")
model_st = SentenceTransformer('all-MiniLM-L6-v2', use_auth_token=False)

# Ürün açıklamalarını anlamsal vektörlere (embedding) dönüştürüyoruz.
print("   - Ürün açıklamaları için embedding'ler oluşturuluyor...")
product_embeddings = model_st.encode(df_products['description'].tolist(), show_progress_bar=True)
embedding_dim = product_embeddings.shape[1]
print(f"   - {embedding_dim} boyutlu embedding'ler oluşturuldu.")

# Embedding'leri bir DataFrame'e dönüştürüyoruz.
embedding_cols = [f'emb_{i}' for i in range(embedding_dim)]
df_embeddings = pd.DataFrame(product_embeddings, columns=embedding_cols)
df_product_embeddings = pd.concat([df_products[['product_id']], df_embeddings], axis=1)

# Ana veri setimize bu embedding'leri ekliyoruz.
df_full = pd.merge(df_full, df_product_embeddings, on='product_id', how='left')
# Eşleşmeyen ürünler için embedding değerlerini 0 ile doldur
df_full[embedding_cols] = df_full[embedding_cols].fillna(0)
print("   - Embedding'ler ana veri setine eklendi.")
...
# Embedding sütunları için agregasyonları (ortalama) dinamik olarak ekle
# Bir oturumdaki tüm ürünlerin anlamsal ortalamasını alarak "oturumun anlamını" temsil eden bir vektör oluşturuyoruz.
for col in embedding_cols:
    agg_dict[col] = (col, 'mean')

session_df = df_full.groupby('user_session').agg(**agg_dict)

session_df = pd.get_dummies(session_df, columns=['main_category'], prefix='cat')
print("Veriler, embedding'ler dahil edilerek oturum bazında birleştirildi.")
...
# Modelin giriş katmanı artık embedding'leri de içereceği için input_shape otomatik olarak ayarlanacaktır.
model = Sequential([
...
submission_df.to_csv('submission_dl_embeddings_final.csv', index=False)

print("\n'submission_dl_embeddings_final.csv' dosyası başarıyla oluşturuldu.")
